  ...
  $.fn.immersive_slider = function (options) {
    ...
    if (settings.cssBlur == true) {
      el.find('.is-slide img:first-child').each(function (index) {
        ...
        $(settings.container)
          .addClass('is-container')
          .prepend(
            "<div id='slide_" +
              (index + 1) +
              "_bg' class='is-background gs_cssblur " +
              activeclass +
              "'>" +
              img.clone().wrap('<div />').parent().html() +
              '</div>'
          )
    ...
    $(settings.container)
      .find('.is-background')
      .wrapAll("<div class='is-bg-overflow' />")
    el.find('.is-slide').wrapAll("<div class='is-overflow' />")
    el.find('.is-slide').each(function (index) {
      var activeclass = ''
      if (index == 0) activeclass = 'active'
      $(this)
        .attr('id', 'slide_' + (index + 1))
    ...
    $.fn.moveSlider = function (settings, page_index) {
      var el = $(this),
        ...
        bg_next = $(settings.container).find(
          '.is-background' + page_index + '_bg'
        )
      if (next) {
        ...
        $('.is-pagination li a' + "[href='" + page_index + "']").addClass(
          'active'
        )
      }
    ...
    $.fn.moveNext = function () {
      var el = $(this),
        ...
        page_index_number =
          parseInt(
            $(this).find('.is-slide.active').attr('id').replace('slide_', '')
          ) + 1
